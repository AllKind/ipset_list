...
- `ipset_list -m -Fg "192.168.*" -Xg "192.168.13.69*" -Gx -Gp none` - Show members of all sets that match the glob "192.168.*", but exclude "192.168.13.69*". Save the query result in xml to an auto generated file. Send the query result in ipset save format to stdout (suppressing normal output).
